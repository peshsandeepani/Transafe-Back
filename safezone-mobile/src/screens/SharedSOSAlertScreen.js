...
function SharedSOSAlertScreen({ sharedSOSAlerts, setScreen, token, user, setSharedSOSAlerts }) {
  ...
  const sortedAlerts = (sharedSOSAlerts || []).sort((a, b) => Number(b.id) - Number(a.id));
  ...
  const handleStartTrip = async () => {
    try {
      ...
      // Fetch routes BEFORE starting trip
  ...
  const handleEndTrip = async () => {
    try {
      ...
      // Remove the alert from shared list after trip completion
            ...
            <>
              <View style={styles.card}>
                ...
                {selectedAlert.senderPhone && (
                  <TouchableOpacity
                    onPress={() => {
                      // Could implement calling functionality here
                      console.log(`Calling ${selectedAlert.senderPhone}`);
                    }}
                  >
                    <Text style={[styles.info, { color: "#0066CC", textDecorationLine: "underline" }]}>
                      <Text style={{ fontWeight: "bold" }}>📱 Contact:</Text> {selectedAlert.senderPhone}
                    </Text>
                  </TouchableOpacity>
                )}
